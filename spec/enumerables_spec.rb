...
describe Enumerable do
  ...
  describe '#my_select' do
    it 'Returns even numbers' do
      expect(array.select(&:even?)).to eql([2, 4, 6, 8])
    end

    it 'Returns words shorter than 4 characters' do
      expect(words_array.select { |n| n.length < 4 }).to eql(%w[ant cat])
    end

    it 'Returns enumerator' do
      expect(array.my_select.class).to be(Enumerator)
    end
  end
  ...
  describe '#my_none?' do
    it 'Returns false for even numbers' do
      expect(array.my_none?(&:even?)).to eql(false)
    end
  ...
  describe '#my_inject' do
    ...
    it 'Returns the substraction of all the array elements starting from three' do
      expect((5..10).my_inject(3) { |sum, num| sum - num }).to eql(-42)
    end
  end
end
